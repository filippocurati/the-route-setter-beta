...
Nella cartella "Pareti" sarà presente il modello 3D della parete su cui tracciare.
Nella cartella "Prese" saranno presenti una cartella per ciascun modello di presa, ogni cartella ha nome "Hold" seguito da un suffisso numerico, ogni cartella contiene i file per la visualizzazione del modello stesso. 
...
La versione corrente dell'applicazione deve gestire una sola parete per sessione, quindi un solo modello 3d della parete. La parete deve essere caricata automaticamente all'avvio dell'applicazione utilizzando il modello disponibile nella cartella "Pareti".
